...
import argparse
from keras.callbacks import LearningRateScheduler, TensorBoard, ModelCheckpoint, Callback, CSVLogger, TerminateOnNaN
...
from adam.losses import dice_loss, rbox_loss
from adam import data_processor
...
# number of steps after which the learning rate is decayed by decay rate
parser.add_argument('--lr_decay_steps', type=int, default=130)
# maximum number of epochs
parser.add_argument('--max_epochs', type=int, default=800)
# list of gpus to use
parser.add_argument('--gpu_list', type=str, default='0')
# path to a directory to save model checkpoints during training
parser.add_argument('--checkpoint_path', type=str, default='checkpoints')
# period at which checkpoints are saved (defaults to every 10 epochs)
parser.add_argument('--save_checkpoint_epochs', type=int, default=3)
...
# minimum size of a text instance; if smaller, then it is ignored during training
parser.add_argument('--min_text_size', type=int, default=10)
# the minimum ratio of min(H, W), the smaller side of the image, when taking a random crop from the input image
parser.add_argument('--min_crop_side_ratio', type=float, default=0.1)
# geometry type to be used; only RBOX is implemented now, but the original paper also uses QUAD
parser.add_argument('--geometry', type=str, default='RBOX')
# whether to show error messages and warnings during training (some error messages during training are expected to
# appear because of the way patches for training are created)
...
class ValidationEvaluator(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if (epoch + 1) % self.period == 0:
            ...
            val_loss_summary.value.add(tag='loss', simple_value=val_loss)
            self.val_writer.add_summary(val_loss_summary, epoch + 1)
            val_score_map_loss_summary = tf.Summary()
            val_score_map_loss_summary.value.add(tag='pred_score_map_loss', simple_value=val_score_map_loss)
            self.val_writer.add_summary(val_score_map_loss_summary, epoch + 1)
            val_geo_map_loss_summary = tf.Summary()
            val_geo_map_loss_summary.value.add(tag='pred_geo_map_loss', simple_value=val_geo_map_loss)
...
def main():
    ...
    train_data_generator = data_processor.generator('train_1125_13_train_15_train_8_8_64_no###.h5',
                                                    dataset_size=train_dataset_size, batch_size=8)
    val_data_generator = data_processor.generator('val_471_15_test_8_8_64_no###.h5',
                                                  dataset_size=val_dataset_size, batch_size=8)
    ...
    csv_logger = CSVLogger(filename='icdar_2015_2013.csv',
                           separator=',',
                           append=True)
    checkpoint = ModelCheckpoint(filepath='icdar_2015_2013_{epoch:02d}_{loss:.4f}_{val_loss:.4f}.h5',
                                 monitor='val_loss',
                                 verbose=1,
    ...
    model.compile(loss=[
        ...
        rbox_loss(east.target_score_map)], loss_weights=[1., 1.], optimizer=opt)
